...
export const DBScan = () => {
  ...
  function euclidean_distance(point1, point2) {
    return Math.sqrt(Math.pow(point2.Age - point1.Age, 2) + Math.pow(point2.Salary - point1.Salary, 2));
  }

  function manhattan_distance(point1, point2) {
    return Math.abs(point2.Age - point1.Age) + Math.abs(point2.Salary - point1.Salary);
  }
    ...
    for (let i = 0; i < num_clusters; i++) {
      if (distance === euclidean_distance || distance === manhattan_distance) {
        ...
        for (let j = 0; j < clusters[i].length; j++) {
          clusters_centers[i].Age += data[clusters[i][j]].Age;
          clusters_centers[i].Salary += data[clusters[i][j]].Salary;
        }

        clusters_centers[i].Age /= clusters[i].length;
        clusters_centers[i].Salary /= clusters[i].length;
